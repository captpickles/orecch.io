...
export function createRepository(config) {
  ...
  async function getSummaryRowsFromEvents(startDate, endDate) {
    ...
    const allEventsByUtcDay = await Promise.all(
      utcFetchKeys.map((dateKey) => client.getEventsByDate(dateKey))
    );
  ...
  async function getSummaryRowsFromDailySummary(startDate, endDate) {
    const dailySummary = await client.getDailySummary();
    const rows = [];
    const types = new Set();
  ...
  async function getDayEvents(dateKey) {
    ...
    const byUtcDay = await Promise.all(
      dedupedKeys.map((utcDateKey) => client.getEventsByDate(utcDateKey))
    );
  ...
  function subscribeDayEvents(dateKey, callback) {
    return client.subscribeEventsByDate(dateKey, callback);
  }
